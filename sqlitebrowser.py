...
path_img = os.path.dirname(os.path.realpath(__file__)) + '/res/database-settings-icon.png'
...
def desk(fp):
    ...
    if len(tables) != 0 :
        combo.current(0)
    combo.place(relx=0.55, rely=0.03 , anchor=NW)
    ...
    btn_exit = Button(window,text='Exit',width=20,command= lambda : exit())
    btn_exit.place(relx=0.67, rely=0.94, anchor=CENTER)
    btn_browse = Button(window,text='Browse',width=20,command= combobx)
